...
import type { OrderTemplateItem, OrderTemplateParty } from "@/components/order-custom-components/order-types";
import {
  SingleSelector,
...
type VoucherAttachment = {
  file_upload_id?: number | string | null;
  file_name: string;
  file_url: string;
  file_size?: number;
  content_type?: string;
...
export function OrderTemplateForm({
  ...
  const [documentDate, setDocumentDate] = React.useState(
    () => stringValue(initialVoucher?.document_date)
  );
  const [status, setStatus] = React.useState(
    () => stringValue(initialVoucher?.status) || "draft"
  );
  ...
  React.useEffect(() => {
    ...
    try {
      ...
      setDocumentDate(stringValue(stored.document_date));
      setStatus(stringValue(stored.status) || "draft");
  ...
  async function uploadAttachment(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    setUploadError(null);
    try {
      const uploaded: VoucherAttachment[] = files.map((file) => ({
        file_upload_id: `mock-file-${file.lastModified}-${file.name}`,
        file_name: file.name,
  ...
  async function saveFinalVoucher(options?: { sendForApproval?: boolean }) {
    ...
    setError(null);
    try {
      const savedPdfPayload: Record<string, unknown> = {
        ...reviewData,
        ...viewEdits,
        details: details.map(({ id, ...row }) => row),
        ...
        work_flow_json: buildWorkflowJson(
          String(reviewData.voucher_uuid ?? `mock-voucher-${Date.now()}`),
      ...
      const savedValues: Record<string, string> = Object.fromEntries(
        Object.entries(savedPdfPayload).map(([key, value]) => [
          key,
          value == null ? "" : typeof value === "string" ? value : JSON.stringify(value),
        ])
      );
      saveVoucherReviewJson(savedPdfPayload);
      ...
      toast.success(
        options?.sendForApproval
          ? "Purchase Order sent for approval in mock mode."
          : "Purchase Order draft saved in mock mode."
      );
      onSaved?.(
        savedValues,
        editingVoucherId ?? Math.floor(Date.now() / 1000),
        savedPdfPayload,
      );
  ...
  React.useEffect(() => {
    if (reviewData?.document_date) setDocumentDate(reviewData.document_date);
    if (reviewData?.status) setStatus(reviewData.status);
  }, [reviewData?.document_date, reviewData?.status]);
  const orderItems = React.useMemo<OrderTemplateItem[]>(
    () => details.map((row) => ({
      id: row.id,
      product: row.ledger,
      quantity: row.credit,
  ...
  const updateOrderItem = (id: number, key: keyof OrderTemplateItem, value: string) => {
    if (key === "product") {
      const product = products.find((item) => item.name === value);
      updateDetail(id, "ledger", value);
      updateDetail(id, "product_uuid", product?.id ?? "");
      return;
    }
    const detailKey: keyof DetailRow = key === "quantity" ? "credit" : key === "price" ? "debit" : "description";
    updateDetail(id, detailKey, value);
  };
  const orderPdfSupplier: OrderTemplateParty = {
    name: selectedContact?.contactName || selectedContact?.name || "",
    company: selectedContact?.companyName || "",
    address: selectedContact?.address || "",
    phone: selectedContact?.mobile || "",
    email: reviewValue("contact_email"),
    taxId: reviewValue("contact_tax_id"),
  };
  const orderPdfBuyer: OrderTemplateParty = {
    name: headerUser.name || "",
    company: headerUser.businessName || "",
    address: headerUser.address || "",
    phone: headerUser.mobile || "",
    email: headerUser.email || "",
    taxId: "",
  };
  const orderPdfProps = {
    orderNumber: reviewValue("voucher_number"),
    documentDate: reviewValue("document_date"),
    supplierDetails: orderPdfSupplier,
    buyer: orderPdfBuyer,
    items: orderItems,
    total: detailTotals.amount,
    supplyTerms: reviewValue("supply_terms"),
    deliveryTerms: reviewValue("delivery_terms"),
    paymentTerms: reviewValue("payment_terms"),
    preparedBy: headerUser.name || "",
    approvalUser: selectedApprover,
  };

  const tabs: Array<[NewTab, string]> = approvedViewOnly
      ? [["voucher", "Purchase Order"], ["json", "PDF View"]]
    : [
        ...(approvalMode ? [] : [["new", "Purchase Order"] as [NewTab, string], ["invoice", "Add Items"] as [NewTab, string]]),
        ["voucher", approvalMode ? "Purchase Order" : "View"],
        ["json", "PDF View"],
      ];
  const tabToStep: Record<NewTab, OrderTemplateStep> = {
    new: "order",
    invoice: "items",
    voucher: approvalMode || approvedViewOnly ? "order" : "view",
    json: "pdf",
  };
  const workflowSteps = tabs.map(([tab, label]) => [tabToStep[tab], label] as [OrderTemplateStep, string]);
  const activeStep = tabToStep[activeTab];
  const changeStep = (step: OrderTemplateStep) => {
    const nextTab = Object.entries(tabToStep).find(([, value]) => value === step)?.[0] as NewTab | undefined;
  ...
  return (
    ...
    >
      <OrderTabs step={activeStep} items={workflowSteps} onStepChange={changeStep} />
      {activeTab === "new" ? (
        <div className={`space-y-5 p-0 sm:p-6 ${sentForApproval ? "pointer-events-none opacity-80" : ""}`}>
          ...
          <section className="rounded-none border-0 bg-transparent p-4 shadow-none sm:rounded-2xl sm:border sm:border-border sm:bg-card sm:p-6 sm:shadow-sm [&>div>div]:content-start">
            <h2 className="mb-5 font-semibold">Purchase Order information</h2>
              ...
              <div className="grid gap-1">
                ...
                >
                  <SingleSelectorTrigger
                    ...
                    className="min-h-10 w-full border border-input bg-background"
                  >
                    <SingleSelectorValue>{selectedContact?.name || undefined}</SingleSelectorValue>
                    <SingleSelectorInput placeholder="Search contacts" />
                  </SingleSelectorTrigger>
                  <SingleSelectorContent>
                    <SingleSelectorList>
                      {contacts.map((contact) => (
                      <SingleSelectorItem key={contact.id} value={contact.id}>
                        {contact.name}
                      </SingleSelectorItem>
                      ))}
                    </SingleSelectorList>
            ...
            <div className="mt-4 grid gap-4 grid-cols-1">
              <div className="grid gap-2">
                ...
                <input type="hidden" name="document_date" value={documentDate} readOnly />
                <Popover>
                  <PopoverTrigger asChild>
                    <Button type="button" variant="outline" className="h-10 w-full justify-between font-normal">
                      <span className={documentDate ? "text-foreground" : "text-muted-foreground"}>
                        {documentDate || "Pick a date"}
                      </span>
                  ...
                  <PopoverContent align="start" className="w-auto p-0">
                    <Calendar
                      mode="single"
                      selected={parseDateInput(documentDate)}
                      onSelect={(value) => {
                        if (value) setDocumentDate(formatDateInput(value));
                      }}
                    />
                  </PopoverContent>
                </Popover>
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] text-muted-foreground">
                          ...
                          {attachment.file_size
                            ? ` Ã‚Â· ${(attachment.file_size / 1024).toFixed(
                                1
                              )} KB`
                            : ""}
                        </p>
        ...
        <div className={sentForApproval ? "pointer-events-none opacity-80" : ""}>
          <AddItems
            supplier={reviewValue("contact_company_name")}
            documentDate={reviewValue("document_date")}
            status={reviewValue("status")}
            orderNumber={reviewValue("voucher_number")}
            items={orderItems}
            products={products.map((product) => product.name)}
            total={detailTotals.amount}
            onUpdateItem={updateOrderItem}
            onRemoveItem={removeDetail}
            onAddItem={addDetail}
            onBack={() => setActiveTab("new")}
            onSaveAndNext={saveDetails}
          />
        </div>
      ) : activeTab === "json" ? (
        <OrderPdfView {...orderPdfProps} />
      ) : (
          ...
          <div className="mx-auto w-full max-w-[1120px]">
            <article className="bg-white px-4 py-8 text-[#193244] shadow-xl sm:px-14 sm:py-12">
              ...
              <div className="mt-8 max-w-md">
                ...
                <SingleSelector value={selectedApprover} onValueChange={updateApprover}>
                  <SingleSelectorTrigger
                    id="purchase-order-approver"
                    aria-label="Search and select approver"
                    className="mt-2 min-h-10 w-full border border-input bg-background"
                  >
                    <SingleSelectorValue>
                      {selectedApprovalUser
                        ? `${selectedApprovalUser.username || selectedApprovalUser.email} (${selectedApprovalUser.email})`
                        : selectedApprover || "Select approver"}
                    </SingleSelectorValue>
                    <SingleSelectorInput placeholder="Search users" />
                  </SingleSelectorTrigger>
                  <SingleSelectorContent>
                    <SingleSelectorList>
                      {approvalUsers.map((approvalUser) => (
                        <SingleSelectorItem key={approvalUser.email} value={approvalUser.email}>
                          {approvalUser.username || approvalUser.email}
                          {approvalUser.username ? ` (${approvalUser.email})` : ""}
                        </SingleSelectorItem>
                      ))}
                    </SingleSelectorList>
                ...
                <div className="flex w-full flex-wrap gap-2 sm:w-auto sm:justify-end">
                  {approvalMode ? (
                    <Button type="button" disabled={approvalApproved || approvalLoading} onClick={onApprove}>
                      {approvalLoading ? "Approving..." : approvalApproved ? "Approved" : "Approve Purchase Order"}
                    </Button>
                  ) : (
                    <>
