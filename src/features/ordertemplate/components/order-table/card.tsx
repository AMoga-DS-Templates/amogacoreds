...
export function OrderCard({ record }: { record: OrderRecord }) {
  return <Card className='min-w-0 rounded-xl'>
    <CardHeader className='flex-row items-start justify-between gap-3 space-y-0 pb-3'><div className='min-w-0'><CardTitle className='break-words text-base'>{record.voucherNumber}</CardTitle><p className='mt-1 text-xs text-muted-foreground'>Order · FY {record.financialYear}</p></div><OrderTableRowMenu record={record} /></CardHeader>
