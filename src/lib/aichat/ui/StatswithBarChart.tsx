...
const StatswithBarChartItemSchema = z.object({
...
const StatswithBarChartSchema = z.object({
  summary: z.array(StatswithBarChartItemSchema).min(1),
});

const toDisplayString = (value: unknown, fallback = "") =>
  value === null || value === undefined ? fallback : String(value);

export const StatswithBarChart = defineComponent({
  name: "StatswithBarChart",
  props: StatswithBarChartSchema,
  description:
    'Responsive stat cards with mini bar charts. summary item: { name, tickerSymbol, value, change, percentageChange, changeType: "positive" | "negative", labels, values }.',
  component: ({ props }) => (
    <div className="w-full max-w-[800px]">
      <dl className="grid w-full grid-cols-[repeat(auto-fit,minmax(min(100%,14rem),1fr))] gap-4 sm:gap-6">
        {props.summary.map((item, index) => {
          ...
          const labels = item.labels.length > 0 ? item.labels : [""];
          const values = item.values.length > 0 ? item.values : [0];
          const color =
